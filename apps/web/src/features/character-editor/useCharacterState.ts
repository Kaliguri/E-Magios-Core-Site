...
function normalizeCharacter(raw: Character): Character {
  ...
  return {
    ...
    temporaryBonuses: Array.isArray(raw.temporaryBonuses) ? raw.temporaryBonuses : [],
...
export function useCharacterState(
      ...
      setCharacter((prev) => {
        ...
        const next = {
          ...
          currentHealth: Math.min(
            prev.currentHealth ?? stats.health,
            prev.maxHealth ?? stats.health,
          ),
  ...
  const updateSkillLevel = useCallback(
    (type: 'magic' | 'personality', skillId: string, level: number) => {
      setCharacter((prev) => {
  ...
  const addSpell = useCallback(
    (type: 'study' | 'signature' | 'spontaneous', spell: CharacterSpell) => {
      setCharacter((prev) => {
        const field =
          type === 'study'
            ? 'studySpells'
  ...
  const removeSpell = useCallback(
    (type: 'study' | 'signature' | 'spontaneous', spellId: string) => {
      setCharacter((prev) => {
        const field =
          type === 'study'
            ? 'studySpells'
        ...
        const next = {
          ...prev,
          [field]: (prev[field] as CharacterSpell[]).filter((s) => s.id !== spellId),
        };
        setIsDirty(true);
        scheduleAutosave(next);
        return next;
